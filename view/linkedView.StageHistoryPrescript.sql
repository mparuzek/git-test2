IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[StageHistoryPrescript]') and OBJECTPROPERTY(id, N'IsView') = 1)
DROP VIEW [dbo].[StageHistoryPrescript]
GO
CREATE VIEW [dbo].[StageHistoryPrescript] AS
SELECT
	[ID],
  ...
  FROM [synonym].[HistoryPrescript]

GO

GO
GRANT SELECT ON [dbo].[StageHistoryPrescript] TO [$ndo$shadow]
